...
export default async function handler(req, res) {
  ...
  if (raw.length < 2) return res.status(400).json({ error: 'Fichier vide' });
  ...
  for (const [collab, moisData] of Object.entries(byCollab)) {
    for (const [moisAnnee, kpis] of Object.entries(moisData)) {
      if ((kpis.jours_potentiels || 0) === 0) continue;
      rows.push({
        mois_annee:         parseInt(moisAnnee),
  ...
  const { data: log } = await db.from('imports_log').insert({
    type_import: 'kpi',
    filename:    file.originalFilename || 'kpi',
    mode:        'fusion',
  ...
  for (let i = 0; i < rows.length; i += CHUNK) {
    ...
    const { error } = await db.from('kpi_mensuels')
      .upsert(chunk, { onConflict: 'mois_annee,collaborateur' });
    if (error) nbMisAjour += chunk.length;
    else nbAjoutes += chunk.length;
  }

  // Générer coûts back-office pour chaque mois
