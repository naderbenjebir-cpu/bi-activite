...
export default async function handler(req, res) {
  ...
  for (const row of rows) {
    const { data: ex } = await db.from('collaborateurs')
      .select('id, cout_jour_manuel, source_cout')
      .eq('nom',    row.nom)
      .eq('prenom', row.prenom || '')
      .maybeSingle();

    if (ex) {
      // Conserver le coût manuel existant
      if (ex.source_cout === 'manuel' && ex.cout_jour_manuel) {
        row.cout_jour_manuel = ex.cout_jour_manuel;
      }
  ...
  await db.from('imports_log').insert({
    type_import: 'salaries',
    filename: file.originalFilename || 'salaries',
    lignes: rows.length,
    summary: `${nbAjoutes} ajoutés, ${nbMisAjour} mis à jour`,
  });
