...
function Toast({ msg, type, onClose }) {
  ...
  return(
    <div style={{position:'fixed',bottom:24,right:24,background:'var(--bg2)',border:`1px solid ${c}`,borderLeft:`4px solid ${c}`,borderRadius:8,padding:'12px 18px',fontSize:13,boxShadow:'0 4px 16px rgba(0,0,0,0.12)',zIndex:200,maxWidth:440}}>
      {msg}<button onClick={onClose} style={{marginLeft:12,background:'none',border:'none',cursor:'pointer',color:'var(--muted)',fontSize:16}}>×</button>
    </div>
  );
}
...
export default function AdminKPI() {
  ...
  async function loadHistory() {
    const {data}=await supabase.from('imports_log').select('*').eq('type_import','kpi')
  ...
  function validateFile(f) {
    if (!f?.name?.startsWith('IndicateursSurActiviteJ_')) {
      showToast(`⚠ Fichier refusé : "${f?.name}"\nAttendu : IndicateursSurActiviteJ_YYYYMMDD.xlsx`,'error');
      return false;
    }
    return true;
  }
  ...
  async function doImport() {
    ...
    try {
      ...
      showToast(`✓ ${data.lignes} entrées KPI importées · ${data.moisList?.length} mois couverts`,'success');
      setFile(null); await loadHistory();
    ...
    <div>
      ...
      <div className="page-body">
        ...
        <div style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:10,padding:'14px 18px',marginBottom:16}}>
          <div style={{fontSize:13,fontWeight:500,marginBottom:4}}>Fichier attendu : IndicateursSurActiviteJ_YYYYMMDD.xlsx</div>
          <div style={{fontSize:12,color:'var(--muted)'}}>
            Format matriciel collaborateurs × indicateurs × mois. Alimente les modules Inter-contrat, Back-Office, TACE et Suivi mission.
          </div>
        </div>

        <div
          style={{border:`1.5px dashed ${dragging||file?'var(--orange)':'var(--border2)'}`,borderRadius:12,padding:'40px 32px',textAlign:'center',cursor:'pointer',background:dragging?'rgba(249,115,22,0.04)':'var(--bg2)',transition:'all .2s'}}
          onClick={()=>fileRef.current?.click()}
          onDragEnter={e=>{e.preventDefault();setDragging(true);}}
          onDragOver={e=>e.preventDefault()}
          onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))setDragging(false);}}
          onDrop={e=>{e.preventDefault();setDragging(false);const f=e.dataTransfer.files[0];if(f&&validateFile(f))setFile(f);}}>
          <input ref={fileRef} type="file" accept=".xlsx,.xls" style={{display:'none'}}
            onChange={e=>{const f=e.target.files[0];if(f&&validateFile(f))setFile(f);e.target.value='';}}/>
          {file?(
            <div>
        ...
        <div className="table-wrap" style={{marginTop:24}}>
          ...
          {history.length===0?<div className="empty"><p>Aucun import KPI enregistré</p></div>:(
