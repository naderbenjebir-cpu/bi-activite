...
const CATEGORIES = ['operationnel', 'back-office'];
const SOUS_TYPES = ['consultant', 'dsi', 'commercial', 'rh', 'dirigeant', 'autre'];
...
function EditCell({ value, onSave, type = 'number', options }) {
  ...
  if (!editing) return (
    <div onClick={() => setEditing(true)}
      style={{cursor:'pointer',padding:'2px 6px',borderRadius:4,minWidth:60,
  ...
  if (options) return (
    <select ref={ref} value={val} onChange={e => setVal(e.target.value)} onBlur={save}
      style={{fontSize:12,padding:'2px 6px',border:'1px solid var(--orange)',borderRadius:4,background:'var(--bg)'}}>
  ...
  return (
    <input ref={ref} type={type} value={val} onChange={e => setVal(e.target.value)}
      ...
      style={{width:80,fontSize:12,padding:'2px 6px',border:'1px solid var(--orange)',borderRadius:4,
...
export default function AdminCollaborateurs() {
  ...
  async function updateCollab(id, field, value) {
    try {
      ...
      const r = await fetch('/api/admin/collaborateur-update', {
        method: 'PATCH',
  ...
  async function importSalaries(file) {
    if (!file.name.startsWith('ListeSalaries_')) {
      showToast(`⚠ Fichier non reconnu : "${file.name}" — attendu : ListeSalaries_YYYYMMDD.xlsx`, 'error');
      return;
    }
  ...
  const manquants = collabs.filter(c => c.actif && !c.cout_jour_effectif).length;

  const btnTab = (k, label) => (
    <button onClick={() => setFilter(k)}
      style={{fontSize:11,padding:'5px 14px',borderRadius:20,cursor:'pointer',fontFamily:'var(--mono)',
        border: filter===k ? 'none' : '1px solid var(--border)',
        background: filter===k ? 'var(--blue)' : 'var(--bg2)',
  ...
  return (
    <div>
      ...
      <div className="topbar">
        ...
        {manquants > 0 && (
          <span style={{marginLeft:12,background:'#fef2f2',color:'#b91c1c',border:'1px solid #fecaca',
            padding:'2px 10px',borderRadius:20,fontSize:11,fontFamily:'var(--mono)'}}>
            {manquants} sans coût
          </span>
        )}
      </div>
      <div className="page-body">
        ...
        <div style={{display:'flex',gap:10,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
          <input ref={fileRef} type="file" accept=".xlsx" style={{display:'none'}}
            onChange={e => e.target.files[0] && importSalaries(e.target.files[0])} />
          <button className="btn btn-primary" onClick={() => fileRef.current?.click()} disabled={importLoading}>
            {importLoading ? 'Import...' : '↑ Importer ListeSalaries.xlsx'}
          </button>
          <div style={{marginLeft:'auto',display:'flex',gap:6}}>
            {btnTab('all',      `Tous (${collabs.length})`)}
            {btnTab('actifs',   `Actifs (${collabs.filter(c=>c.actif).length})`)}
                ...
                <tbody>
                  {filtered.map(c => (
                    <tr key={c.id}>
                      ...
                      <td>
                        <EditCell value={c.categorie} options={CATEGORIES}
                          onSave={v => updateCollab(c.id, 'categorie', v)} type="text" />
                      </td>
                      <td>
                        <EditCell value={c.sous_type} options={SOUS_TYPES}
                          onSave={v => updateCollab(c.id, 'sous_type', v)} type="text" />
                      </td>
                      <td className="td-muted">{c.service||'—'}</td>
                      <td style={{fontSize:11,fontFamily:'var(--mono)'}}>
                        {(c.type_contrat||'').replace('CDI - Contrat à durée indéterminée','CDI').replace('CDD - Contrat à durée déterminée','CDD').slice(0,6)}
                      </td>
                      <td>
                        <EditCell value={c.cout_jour_manuel}
                          onSave={v => updateCollab(c.id, 'cout_jour_manuel', v)} />
                      </td>
                      <td className="td-right" style={{fontFamily:'var(--mono)',fontSize:11}}>
                        {c.cout_jour_effectif ? `${fmtNum(c.cout_jour_effectif,0)} €` : '—'}
                      </td>
                      <td>
                        <span className={`badge ${c.source_cout==='manuel'?'badge-orange':'badge-gray'}`}>
                          {c.source_cout||'—'}
                        </span>
                      </td>
                      <td>
                        {c.dernier_statut && (
                          <span className={`badge ${
                            c.dernier_statut==='mission'?'badge-green':
                            c.dernier_statut==='inter-contrat'?'badge-red':
                            c.dernier_statut==='mission-partielle'?'badge-orange':'badge-gray'
                          }`}>{c.dernier_statut}</span>
                        )}
                      </td>
                    </tr>
                  ))}
